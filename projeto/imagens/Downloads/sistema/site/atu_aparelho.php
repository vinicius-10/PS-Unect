...
$sql= $db -> prepare("SELECT nome_aparelho, relay,img from aparelho where id_aparelho=$id_aparelho");
$sql -> execute();
$sql -> bind_result($nome, $relay,$img);
$sql -> fetch();
$sql -> close();

if(isset($_GET['c'])){
    echo "    <div class='popup' > <a href='atu_aparelho.php?id=$id_aparelho'><img class= fechar src=./imagens/fechar.png onclik=fechar() width='30px'></a> <div class='mensagem'>Aparelho atualizado com susceso<br><br><img src='imagens/sucesso.png' width='70'> </div> </div>";
}
    ...
    <div class="from">
        ...
        <form action="atualizar.php?id=<?php echo $id_aparelho ?> " method= "POST" enctype="multipart/form-data">
            ...
            <div class="field">
                ...
                <input type="text" id="nome" placeholder="Nome" name='nome' required maxLength=50 value='<?php echo $nome ?>'>
            </div>
            ...
            <div class="image">
                <label for="file" class="file">Imagem</label>
                <input type='file' id='file' name='file' accept="image/*" placeholder="Imagem" ><br>
            
                <img src="<?php echo $img ?>" id="image" >
            </div>
            
            <div class="field">
                <button type='submit' name='aparelho'>Atualizar</button>
